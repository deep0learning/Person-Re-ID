...
class Pretrain_GoogleNet(object):
    ...
    def __init__(self):
        self.googlenet = MyGoogleNet.inception_v3(pretrained=True, num_classes=cfg.num_classes)
        self.reid_criterion = nn.CrossEntropyLoss()
        self.optimReID = torch.optim.SGD(self.googlenet.parameters(), lr=0.0015, momentum=0.9, weight_decay=0.0002)
    ...
    def pretrain_reid(self):
        ...
        for ep in range(start_ep, cfg.pre_reid_num_epochs):
            ...
            while not epoch_done:
                ...
                step += 1
                step_st = time.time()
                ims, im_names, labels, ims_mirrored, epoch_done = self.train_set.next_batch()
                ims = TVT(Variable(torch.from_numpy(ims).float()))
                labels = TVT(Variable(torch.LongTensor(labels)))
                logits, aux = self.googlenet(ims)
